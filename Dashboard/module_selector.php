...
<div class="" style="">
  ...
  <div id="moduleControls">
    <div class="row g-2 controls-row mb-2">
      <div class="col-md-6">
        <input id="moduleSearchInput" class="form-control" placeholder="Search name / code / year / semester..." />
      </div>
      <div class="col-md-2">
      ...
      <div class="col-md-2 text-end">
        <a href="timetable_set.php#modules" onclick="window.location.reload(); return false;" class="btn btn-outline-secondary btn-sm w-100">
...
<script>
// Function to show no groups selected message
      ...
      <tr>
        <td colspan="7" class="text-center py-4">
          <div class="alert alert-warning mb-0">
            Please select at least one group from the timetable to view modules.
            <a href="timetable_set.php#modules" class="alert-link ms-2" onclick="window.location.reload();">Get modules</a>
          </div>
        </td>
      </tr>
    `;
...
(async () => {
  ...
  // Check if we have selected groups in localStorage
  try {
    ...
    console.error('Error checking selected groups:', e);
    showNoGroupsMessage();
    return;
  }
  ...
  function initApp() {
    ...
    const searchInput = el('moduleSearchInput');
  ...
  function initApp() {
    ...
    function renderSelectedModule() {
      ...
      try {
        ...
        const nameEl = selectedCard.querySelector('#sel_name');
        const codeEl = selectedCard.querySelector('#sel_code');
        const creditsEl = selectedCard.querySelector('#sel_credits');
        const yearEl = selectedCard.querySelector('#sel_year');
        const semEl = selectedCard.querySelector('#sel_semester');
        const programEl = selectedCard.querySelector('#sel_program');

        if (nameEl) nameEl.textContent = module.name || '';
        if (codeEl) codeEl.textContent = module.code || 'N/A';
        if (creditsEl) creditsEl.textContent = module.credits ?? 'N/A';
        if (yearEl) yearEl.textContent = module.year ?? 'N/A';
        if (semEl) semEl.textContent = module.semester ?? 'N/A';
        if (programEl) programEl.textContent = module.program_name || module.program_id || 'N/A';
      } catch (e) {
  ...
  // Only initialize the app if we have selected program IDs
  if (selectedProgramIds && selectedProgramIds.length > 0) {
    initApp();
  } else {
    // Show message if no modules are available for selected groups
    const modulesTbody = document.getElementById('modulesTbody');
    if (modulesTbody) {
      modulesTbody.innerHTML = `
        <tr>
          <td colspan="7" class="text-center py-4">
            <div class="alert alert-warning mb-0">
              No modules found for the selected groups.
              <a href="timetable_set.php" class="alert-link ms-2">Back to Timetable</a>
            </div>
          </td>
        </tr>
      `;
