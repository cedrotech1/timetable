...
<style>
body { background: #f4f6f8; padding: 5px; font-size: 12px; }
...
.table-dark { background-color: #031f50; color: white; font-size: 12px; }
...
function updateFilters() {
  ...
  let schools = selectedCollege ? groups.filter(g=>g.college?.id==selectedCollege).map(g=>g.school) : groups.map(g=>g.school);
  setOptions(filters.school, schools.filter(Boolean), g=>g.id);

  let selectedSchool = filters.school.value;
  let departments = selectedSchool ? groups.filter(g=>g.school?.id==selectedSchool).map(g=>g.department) : groups.map(g=>g.department);
  setOptions(filters.department, departments.filter(Boolean), g=>g.id);

  let selectedDept = filters.department.value;
  let programs = selectedDept ? groups.filter(g=>g.department?.id==selectedDept).map(g=>g.program) : groups.map(g=>g.program);
  setOptions(filters.program, programs.filter(Boolean), g=>g.id);

  let selectedProg = filters.program.value;
  let intakes = selectedProg ? groups.filter(g=>g.program?.id==selectedProg).map(g=>g.intake) : groups.map(g=>g.intake);
  setOptions(filters.intake, intakes.filter(Boolean), g=>g.id);

  let selectedIntake = filters.intake.value;
  let groupList = selectedIntake ? groups.filter(g=>g.intake?.id==selectedIntake) : groups;
