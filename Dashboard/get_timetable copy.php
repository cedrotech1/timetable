...
SELECT 
    ...
    -- Campus (can come from college hierarchy)
    COALESCE(col_department.campus_id, col_program.campus_id) AS campus_full_id,
    COALESCE(camp_department.name, camp_program.name) AS campus_full_name
...
LEFT JOIN campus camp_department ON col_department.campus_id = camp_department.id
...
LEFT JOIN campus camp_program ON col_program.campus_id = camp_program.id
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($row['group_id'] && !isset($data[$tid]['groups'][$group_key])) {
        ...
        // Add school, college, and campus (these will always exist)
        $group_data['school'] = [
            'id' => $row['school_id'],
        ...
        $group_data['campus'] = [
            'id' => $row['campus_full_id'],
            'name' => $row['campus_full_name']
        ];
