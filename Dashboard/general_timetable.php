...
// Set up display names based on user role
$school = null;
$displayName = 'All Schools Timetable';  // Default title

// For registrar users, show campus name if available
if ($role === 'registrar_office' && $campusId) {
    $stmt = $connection->prepare("SELECT name FROM campus WHERE id = ?");
    $stmt->bind_param("i", $campusId);
    $stmt->execute();
    $campus = $stmt->get_result()->fetch_assoc();
    $displayName = $campus ? ($campus['name'] . ' Campus Timetable') : 'Campus Timetable';
} 
// For school users, show school name
else if ($schoolId) {
    $stmt = $connection->prepare("SELECT name FROM school WHERE id = ?");
    $stmt->bind_param("i", $schoolId);
    $stmt->execute();
    $school = $stmt->get_result()->fetch_assoc();
    $displayName = $school ? ($school['name'] . ' Timetable') : 'School Timetable';
}
...
<br>
<!-- Dynamic view info -->
<h2 class="fw-semibold timetable-title mt-2"><?php echo htmlspecialchars($displayName) ?></h2>
...
async function loadTimetable() {
  try {
    ...
    // Use all data without school or campus filtering
    apiData = json.data || [];
...
function updateFilters() {
  ...
  // Update college and school filters
  updateCollegeAndSchoolFilters();
}

// Update college and school filters based on selected program/year/group
function updateCollegeAndSchoolFilters() {
  // Get all unique colleges and schools from the filtered groups
  const allColleges = [];
  const allSchools = [];
  ...
  // Filter groups based on current selections
  const filteredGroups = apiData.flatMap(t => t.groups || []).filter(g => {
    if (currentProgram && g.program?.id != currentProgram) return false;
    if (currentYear && g.intake?.year_of_study != currentYear) return false;
    return true;
  });

  // Get unique colleges and schools from filtered groups
  filteredGroups.forEach(g => {
    if (g.college && g.college.id && !collegeMap.has(g.college.id)) {
      collegeMap.set(g.college.id, g.college);
      allColleges.push(g.college);
    }
    if (g.school && g.school.id && !schoolMap.has(g.school.id)) {
      schoolMap.set(g.school.id, g.school);
      allSchools.push(g.school);
    }
  });

  // Sort colleges and schools by name
  allColleges.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  allSchools.sort((a, b) => (a.name || '').localeCompare(b.name || ''));

  // Update dropdowns with proper function parameters
  setOptions(filters.college, allColleges, item => item.id, item => item.name || 'Unnamed');
  setOptions(filters.school, allSchools, item => item.id, item => item.name || 'Unnamed');

  // Restore selections if they're still valid
  if (currentCollege && allColleges.some(c => c.id == currentCollege)) {
    filters.college.value = currentCollege;
  } else if (filters.college.value) {
    filters.college.value = '';
    filters.school.value = ''; // Reset school if college changes
  }

  if (currentSchool && allSchools.some(s => s.id == currentSchool)) {
    filters.school.value = currentSchool;
  } else if (filters.school.value) {
  ...
  const filteredData = apiData
    ...
    .map(t => ({
      ...
      groups: (t.groups || []).filter(gr => {
        ...
        const collegeMatch = !college || (gr.college && gr.college.id == college);
        const schoolMatch = !school || (gr.school && gr.school.id == school);
        
        return yearMatch && programMatch && groupMatch && collegeMatch && schoolMatch;
      })
    }))
...
function renderTable(data) {
  ...
  data.forEach(t => {
    ...
    const viewUrl = `timetable_details_all.php?id=${t.id}`;
