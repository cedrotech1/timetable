...
if (isset($_POST['action'])) {
    if ($_POST['action'] == 'get_colleges') {
        ...
        // For admin role, set campus, college, and school to NULL
        if ($role === 'admin') {
            $query = "UPDATE users SET role = ?, campus = NULL, college = NULL, school = NULL WHERE id = ?";
            $stmt = $connection->prepare($query);
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-2">
                        <select name="role" class="form-select">
                            ...
                            <option value="registrar_office" <?php echo (isset($_GET['role']) && $_GET['role'] === 'registrar_office') ? 'selected' : ''; ?>>Registrar Office</option>
                            <option value="dean_office" <?php echo (isset($_GET['role']) && $_GET['role'] === 'dean_office') ? 'selected' : ''; ?>>Dean Office</option>
                            <option value="dtle" <?php echo (isset($_GET['role']) && $_GET['role'] === 'dtle') ? 'selected' : ''; ?>>DTLE</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-primary">Search</button>
                        <?php if(isset($_GET['search']) || isset($_GET['phone']) || isset($_GET['role'])): ?>
                            <a href="add_user.php" class="btn btn-secondary">Clear</a>
                        <?php endif; ?>
                    </div>
                    <div class="col-md-2">
                        <a href="add_new_user.php" class="btn btn-primary">add new staff</a>
            ...
            <tbody>
                ...
                while ($user = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($user['ur_email'] ?? ''); ?></td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#editRole').change(function() {
                const role = $(this).val();
                if (role === 'admin') {
                    $('#editCampus').val('').trigger('change');
                    $('#editCollege').val('').trigger('change');
                    $('#editSchool').val('').trigger('change');
                    $('.campus-field, .college-field, .school-field').hide();
                } else {
                    $('.campus-field, .college-field, .school-field').show();
                    toggleFields(role, $('#editCampus').val(), $('#editCollege').val(), $('#editSchool').val());
                }
            });

            // Function to load all colleges
            function loadAllColleges() {
                $.ajax({
                    url: 'add_user.php',
                    method: 'POST',
                    data: { action: 'get_colleges' },
            ...
            function toggleFields(role, campus = '', collegeId = '', schoolId = '') {
                $('#collegeField').addClass('d-none');
                $('#schoolField').addClass('d-none');
                $('#editSchool').html('<option value="">Select School</option>');

                if (role === 'dean_office') {
                    // Only show college and school fields for dean_office role
                    loadAllColleges();
                    
                    // If collegeId is provided, select it and load its schools
                    if (collegeId) {
                        $('#editCollege').val(collegeId).trigger('change');
                    }
                } else {
                    // For other roles, clear and hide college and school fields
                    $('#editCollege').val('').trigger('change');
                    $('#editSchool').val('');
                    $('#collegeField').addClass('d-none');
                    $('#schoolField').addClass('d-none');
            ...
            $('#saveUserBtn').click(function() {
                ...
                const campus = role === 'admin' ? '' : $('#editCampus').val();
                const college = (role === 'admin' || role !== 'dean_office') ? '' : $('#editCollege').val();
                const school = (role === 'admin' || role !== 'dean_office') ? '' : $('#editSchool').val();
                ...
                // Only include college and school for dean_office role
                if (role === 'dean_office') {
