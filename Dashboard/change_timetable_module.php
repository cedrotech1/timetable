...
<body>
          ...
          <div class="card">
            <div class="card-body">
              <?php if (!empty($timetable)): ?>
                <h5 class="card-title current-module-display">
                  Current Module: <?= htmlspecialchars($timetable['module_code'] ?? 'N/A') ?> - <?= htmlspecialchars($timetable['module_name'] ?? 'N/A') ?>
                </h5>
                ...
                <div class="table-responsive">
                  <table id="modulesTable" class="table table-striped table-hover">
                    ...
                    <tbody>
                      <?php
                      if ($programId > 0) {
                        $stmt = $connection->prepare("SELECT m.* FROM module m WHERE m.program_id = ? ORDER BY m.code");
                        $stmt->bind_param("i", $programId);
                        $stmt->execute();
                        $result = $stmt->get_result();
                        while ($module = $result->fetch_assoc()) {
                          $isCurrent = ($module['id'] == ($timetable['module_id'] ?? 0));
                          echo "<tr data-module-id='{$module['id']}'>";
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        Are you sure you want to change the module to <strong><span id="moduleName"></span></strong>?
                      </div>
  ...
  <div class="position-fixed bottom-0 end-0 p-3" style="z-index: 1200">
    <div id="successToast" class="toast align-items-center text-white bg-success border-0" role="alert">
      <div class="d-flex">
        <div class="toast-body" id="successToastMessage">Module updated successfully!</div>
  ...
  $(document).ready(function() {
      ...
      $('#confirmChange').on('click', function() {
          ...
          $.ajax({
              ...
              type: 'POST',
              data: { module_id: selectedModuleId, timetable_id: <?= $timetableId ?> },
              dataType: 'json',
              beforeSend: function() {
                  console.log('Updating module:', { module_id: selectedModuleId, timetable_id: <?= $timetableId ?> });
              },
              success: function(response) {
                  if (response.success) {
                      // Update UI
                      $('.current-module-display').text(
                          'Current Module: ' + response.module_code + ' - ' + response.module_name
                      );

                      const $row = $(`tr[data-module-id="${selectedModuleId}"]`);
                      $row.find('td:last-child').html('<span class="badge bg-success">Current Module</span>');
                      $row.addClass('table-success');

                      // Remove other "Current" badges
                      $('td .badge.bg-secondary').text('Select').removeClass('bg-secondary').addClass('bg-primary')
                          .wrapInner('<button class="btn btn-sm btn-primary select-module"></button>')
                          .find('button').attr({
                              'data-module-id': selectedModuleId,
                              'data-module-name': response.module_code + ' - ' + response.module_name
                          });
                      ...
                      $('#errorToastMessage').text(response.message || 'Update failed.');
      ...
      // Fix focus issue on modal hide
      $('#confirmModal').on('hidden.bs.modal', function () {
          $('.select-module').first().focus();
      });
  });
  </script>
  ...
  <script src="assets/vendor/php-email-form/validate.j  s"></script>
