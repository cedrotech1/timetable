...
// Get all colleges and schools for the campus (or all campuses if admin)
function getCampusCollegesAndSchools($connection, $campus_id = null) {
    $data = ['colleges' => [], 'schools' => []];
    
    // Get colleges
    $college_query = "SELECT c.*, camp.name as campus_name 
                     FROM college c 
                     JOIN campus camp ON c.campus_id = camp.id";
    
    if ($campus_id) {
        $college_query .= " WHERE c.campus_id = " . (int)$campus_id;
    }
    
    $college_query .= " ORDER BY camp.name, c.name";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_schools'])) {
    ...
    if ($site_id > 0) {
        ...
        try {
            ...
            if (!empty($schools)) {
                // Verify each school belongs to the site's campus before assignment
                $placeholders = rtrim(str_repeat('?,', count($schools)), ',');
                $school_ids = array_map('intval', $schools);
                
                // Get valid schools that belong to the site's campus
                $valid_schools_query = "SELECT s.id 
                                      FROM school s 
                ...
                $types = str_repeat('i', count($schools)) . 'i';
                $params = array_merge($school_ids, [$campus_id]);
                $stmt->bind_param($types, ...$params);
                $stmt->execute();
                $valid_schools = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
                
                // Only insert valid school assignments
