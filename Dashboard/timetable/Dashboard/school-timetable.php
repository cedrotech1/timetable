...
function updateCurrentView() {
  ...
  if (filters.department.value) {
    if (filters.department.value === 'null') {
      html += `<strong>Department:</strong> No Department<br>`;
    } else {
      html += `<strong>Department:</strong> ${filters.department.options[filters.department.selectedIndex].text}<br>`;
    }
  }
...
async function loadTimetable() {
  try {
    ...
    // Filter data based on user's school ID
    apiData = (json.data || []).map(t => {
      t.groups = (t.groups || []).filter(g => {
        // Filter by campus and school
        const campusMatch = g.campus?.id == <?php echo $campusId ?? 0; ?>;
...
function updateFilters() {
  ...
  // Department filter - show all departments in the school, including null (for programs directly under school)
  let departments = groups.map(g => g.department).filter(d => d !== undefined);
  setOptions(filters.department, departments, d => d ? d.id : 'null');

  // Program filter - depends on department selection
  let selectedDept = filters.department.value;
  let programs = [];
  
  if (selectedDept === 'null') {
    // Programs directly under school (no department)
    programs = groups.filter(g => !g.department).map(g => g.program);
  ...
  setOptions(filters.program, programs.filter(Boolean), p => p.id);

  // Intake filter - depends on program selection
  ...
  setOptions(filters.intake, intakes.filter(Boolean), i => i.id);

  // Group filter - depends on intake selection
...
function filterData() {
  const department = filters.department.value;
    ...
    .map(t => {
      const g = (t.groups || []).filter(gr => {
        const deptMatch = !department || 
                         (department === 'null' ? !gr.department : gr.department?.id == department);
        return deptMatch &&
    ...
    sessionKeys.forEach((key, idx) => {
      ...
      groups.forEach((g, gIdx) => {
        ...
        tableBody.innerHTML += `<tr>
          ...
          <td rowspan="${rowspan * sessionKeys.length}"><a href="timetable_details_school.php?id=${t.id}" class="btn btn-primary btn-sm">View</a></td>` : ''}
        </tr>`;
      });
    });
  });
